$(function () {
  ...
  var dt_currency_table = $('.datatables-currencies');

  try {
    // Inicializa DataTable si el elemento existe
    if (dt_currency_table.length) {
      var dt_currencies = dt_currency_table.DataTable({
        ajax: {
        ...
        language: {
          ...
          infoFiltered: 'filtrados de _MAX_ monedas',
          paginate: {
            first: '<<',
            last: '>>',
            next: '>',
        ...
        initComplete: function () {
          ...
          this.api()
            ...
            .every(function () {
              var column = this;
              var select = $('<select class="form-select"><option value="">Todos los códigos</option></select>')
                .appendTo('.code_filter')
                .on('change', function () {
      ...
      $('.toggle-column').on('change', function() {
        var column = dt_currencies.column($(this).attr('data-column'));
      ...
      $('#checkAll').on('change', function () {
        var checkboxes = $('.datatables-currencies tbody input[type="checkbox"]');
        checkboxes.prop('checked', $(this).prop('checked'));
        toggleActionsMenu();
      });

      // Activar desactivar checkbox principal
      $('.datatables-currencies tbody').on('change', 'input[type="checkbox"]', function () {
        toggleActionsMenu();
        var allChecked =
          $('.datatables-currencies tbody input[type="checkbox"]').length ===
          $('.datatables-currencies tbody input[type="checkbox"]:checked').length;
        $('#checkAll').prop('checked', allChecked);
      });

      // Eliminar filtros de búsqueda
      $(document).on('click', '#clear-filters', function () {
        $('.code_filter select').val('').trigger('change');
        $('#startDate').val('');
        $('#endDate').val('');
        dt_currencies.search('');
      ...
      function toggleActionsMenu() {
        ...
        var selectedCount = $('.datatables-currencies tbody input[type="checkbox"]:checked').length;
        if (selectedCount >= 2) {
